...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='CompVis/stable-diffusion-v1-4',concept_name=None, latent_mode=False):
        super().__init__()
    ...
    def train_step(self, text_embeddings, inputs, guidance_scale=100):
        ...
        # latents = F.interpolate(latents, (64, 64), mode='bilinear', align_corners=False)
            ...
            # print(f"input type {inputs.type()}")
    ...
    def decode_latents(self, latents):
        ...
        # latents.shapetorch.Size([1, 4, 160, 160])
        # latents = F.interpolate(latents, (64, 64), mode='bilinear', align_corners=False)
        latents = 1 / 0.18215 * latents

        with torch.no_grad():
            imgs = self.vae.decode(latents).sample
            # print(f"decoder?{self.vae.decode}")

        imgs = (imgs / 2 + 0.5).clamp(0, 1)
        # print(f"image.shape{imgs.shape}")
        # image.shapetorch.Size([1, 3, 1280, 1280])
        return imgs
