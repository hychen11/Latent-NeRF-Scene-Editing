...
class fg_Trainer(object):
    ...
    def train_step(self, data):
        ...
        loss_guidance = self.guidance.train_step(text_z, pred_rgb,as_latent=as_latent)
        loss=loss_guidance

        # regularizations
        # 5e-4
        pred_wd=outputs['weights_sum'].reshape(B,1,H,W)
        loss += self.opt.lambda_sparsity * self.losses['sparsity_loss'](pred_wd)

        # Shape loss  5e-6
        loss += self.opt.lambda_shape * self.losses['shape_loss'](outputs['xyzs'], outputs['sigmas'])
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            loss=self.train_one_epoch(train_loader, max_epochs)
    ...
    def init_losses(self) -> Dict[str, Callable]:
        ...
        cfg=TrainConfig()
        losses['shape_loss'] = ShapeLoss(cfg.guide)
        return losses