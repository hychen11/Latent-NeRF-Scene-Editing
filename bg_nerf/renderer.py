...
class NeRFRenderer(nn.Module):
    ...
    #max_steps is the max number of sampled points along each ray
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            # print(f"xyzs dirs {xyzs.shape, dirs.shape}")
            ...
            # print(f"sigmas rgbs {sigmas.shape, rgbs.shape}")
    ...
    #max_steps is the max number of sampled points along each ray
    def run_cuda1(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
    ...
    # max_steps is the max number of sampled points along each ray
    def run_cuda2(self, rays_o1, rays_d1, rays_o2, rays_d2, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]

        prefix = rays_o1.shape[:-1]
        rays_o1 = rays_o1.contiguous().view(-1, 3)
        rays_d1 = rays_d1.contiguous().view(-1, 3)
        rays_o2 = rays_o2.contiguous().view(-1, 3)
        rays_d2 = rays_d2.contiguous().view(-1, 3)

        N1, N2 = rays_o1.shape[0], rays_o2.shape[0]  # N1, N2 = B * N, in fact
        N = N1 + N2
        device = rays_o1.device

        # pre-calculate near far
        nears1, fars1 = raymarching.near_far_from_aabb(rays_o1, rays_d1, self.aabb_infer, self.min_near)
        nears2, fars2 = raymarching.near_far_from_aabb(rays_o2, rays_d2, self.aabb_infer, self.min_near)
        nears = torch.cat((nears1, nears2), dim=0)
        fars = torch.cat((fars1, fars2), dim=0)

        # # mix background color
        # if self.bg_radius > 0:
        #     # use the bg model to calculate bg_color
        #     sph = raymarching.sph_from_ray(torch.cat((rays_o1, rays_o2), dim=0), torch.cat((rays_d1, rays_d2), dim=0), self.bg_radius) # [N, 2] in [-1, 1]
        #     bg_color = self.background(sph, torch.cat((rays_d1, rays_d2), dim=0)) # [N, 3]
        # elif bg_color is None:
        #     bg_color = 1
        ...
        weights_sum = torch.zeros(N, dtype=dtype, device=device)
        depth = torch.zeros(N, dtype=dtype, device=device)
        image1 = torch.zeros(N, 3, dtype=dtype, device=device)
        image2 = torch.zeros(N, 3, dtype=dtype, device=device)
        n_alive = N
        rays_alive = torch.arange(n_alive, dtype=torch.int32, device=device) # [N]
        rays_t1 = nears.clone() # [N]
        rays_t2 = nears.clone() # [N]
        ...
        while step < max_steps:
            ...
            # march rays for rays_o1, rays_d1
            xyzs1, dirs1, deltas1 = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t1, rays_o1, rays_d1, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)
            sigmas1, rgbs1 = self(xyzs1, dirs1)
            sigmas1 = self.density_scale * sigmas1

            # march rays for rays_o2, rays_d2
            xyzs2, dirs2, deltas2 = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t2, rays_o2, rays_d2, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)
            sigmas2, rgbs2 = self(xyzs2, dirs2)
            sigmas2 = self.density_scale * sigmas2

            # composite rays for rays_o1, rays_d1
            raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t1, sigmas1, rgbs1, deltas1, weights_sum, depth, image1, T_thresh)
            print("1st finish")
            # composite rays for rays_o2, rays_d2
            raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t2, sigmas2, rgbs2, deltas2, weights_sum, depth, image2, T_thresh)
            print("2nd finish")

            rays_alive = rays_alive[rays_alive >= 0]

            print(f'step = {step}, n_step = {n_step}, n_alive = {n_alive}, xyzs: {xyzs1.shape}')
        ...
        results['depth'] = depth
        results['image'] = image
        results['sigmas1'] = sigmas1
        results['sigmas2'] = sigmas2
        results['rgbs1'] = rgbs1
        results['rgbs2'] = rgbs2
