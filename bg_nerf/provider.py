...
def rand_poses(size, device, radius=1, theta_range=[np.pi/3, 2*np.pi/3], phi_range=[0, 2*np.pi]):
    ...
    #turn the (thetas,phis,radius) into (x,y,z)
...
class bg_NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        # print(f"self.num_rays{self.num_rays}")

        self.rand_pose = opt.rand_pose
        print(opt.path,opt.preload,opt.scale,opt.offset,opt.bound,opt.fp16)
        ...
        if self.mode == 'colmap':
            ...
            # only load one specified split
                ...
                print(f"type{ type,self.root_path}")
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            # for f in frames:
                ...
                # print(f"image.shape[1]{image.shape[1]}")
                ...
                # self.H=140
                # self.W=216
                print(f"H={self.H},W={self.W},C={image.shape[-1]}??")
                ...
                # print(f"!!!! 1 4096 4??为什么是4{image.shape}")
        ...
        print(f"np.array([fl_x, fl_y, cx, cy]) is {np.array([fl_x, fl_y, cx, cy])}")
    ...
    def dataloader(self):
        ...
        loader.intrinsics=self.intrinsics
        loader.poses=self.poses
