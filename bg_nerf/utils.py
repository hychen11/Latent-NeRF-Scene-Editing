...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1):
    ...
    """
    The values in i and j range from 0 to H-1 and 0 to W-1, respectively,
     and correspond to the row and column indices of each pixel in the image grid.
    """
    results = {}
    ...
    if N > 0:
        ...
        #     #scaling factors for mapping the indices to the original image resolutionaa
        #     sx, sy = H / 128, W / 128
    ...
    #inds is used to update the error map
    # print(f"i{i}")
    # print(f"j{j}")
...
class bg_Trainer(object):
        ...
        if self.workspace is not None:
            if self.use_checkpoint == "scratch":
                ...
                if os.path.exists(self.best_path):
                    ...
                    self.load_checkpoint()
            elif self.use_checkpoint == "chy":
                print(5)
                self.log(f"[INFO] loading chy model ...")
                self.load_checkpoint(checkpoint='/data/hychen/ln_data/test/checkpoints/ngp_ep0300.pth')
            else: # path to ckpt
    ...
    def train_step(self, data):
        ...
        # if there is no gt image, we train with CLIP loss.(text embedding bound)
   
        # if 'images' not in data:
        #     # print("'images' not in data")
        #     B, N = rays_o.shape[:2]
        #     H, W = data['H'], data['W']
        ...
        # print("'images' in data")
        images = data['images'] # [B, N, 3/4]
        print(f"images.shape{images.shape}")
        # 1 4096 4??? (num_rays 4096) 4=3+1?? rgb+density
        B, N, C = images.shape
        # print(f"1111111111111111111B{B},N{N},C{C}")
        ...
        # print(f"111{pred_rgb.shape}222{gt_rgb.shape}")
        # print(f"pred_rgb{pred_rgb.shape}")
    ...
    def train(self, train_loader, valid_loader=None, max_epochs=0):
    ...
    #add another coordinate (主要test step)
    #800 * 800 * 3 res
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        # print(f"pred shape {all_preds[0].shape}")
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        # checkpoint_path='/home/hychen/latent-nerf/0313/checkpoints/ngp_ep0300.pth'
