...
from bg_nerf.utils import bg_Trainer,get_rays

from fg_nerf.provider import fg_NeRFDataset
from fg_nerf.network import fg_NeRFNetwork
from fg_nerf.utils import fg_Trainer,get_rays
...
class Generator(nn.Module):
    ...
    def get_transformations(self, val_s=[[0.5, 0.5, 0.5]],
        ...
        device = self.device
        # val_s=val_s.detach().numpy()
        # val_t=val_t.detach().numpy()
        # val_r=val_r.detach().numpy()
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
        ...
        pixels = arange_pixels((H, W), batch_size, invert_y_axis=False)[1].to(device)
        pixels[..., -1] *= -1.
        ...
        all_preds2 = []
        acc_maps = []
        bg_opt.path= "/data/hychen/final/step3/LN_lego"
        ...
        # bg_checkpoint_path='/data/hychen/ln_data/chair_scene/checkpoints/ngp_ep0300.pth'
        # bg_checkpoint_path='/data/hychen/count_result/checkpoints/ngp_ep0143.pth'
        bg_checkpoint_path='/data/hychen/final/step1_garden/checkpoints/ngp_ep0187.pth'
        # bg_checkpoint_path='/data/hychen/final/step1_counter/checkpoints/ngp_ep0143.pth'

        bg_trainer = bg_Trainer(name='ngp' , opt=bg_opt, model=model_bg, device='cuda',use_checkpoint="chy")
        ...
        # fg_checkpoint_path='/data/hychen/final/step2.2/dog/checkpoints/step_005000.pth'
        ...
        model_fg = fg_NeRFNetwork(fg_opt).to('cuda')
        # # fg_checkpoint_path='/data/hychen/ln_data/dog_obj/checkpoints/df_ep0099.pth'
        # # fg_checkpoint_path='/data/hychen/ln_data/test/Dreamfusion_sd/checkpoints/df_ep0100.pth'
        # # fg_checkpoint_path='/data/hychen/experiment/dog_0421/checkpoints/df_ep0100.pth'
        # fg_checkpoint_path='/data/hychen/stable-dreamfusion-main/workspace_noshape/checkpoints/df_ep0100.pth'
        # fg_checkpoint_path='/data/hychen/final/step2.1/lego/checkpoints/df_ep0100.pth'
        ...
        # fg_trainer.model.load_state_dict(torch.load(fg_checkpoint_path)['model'],strict=False)
        fg_trainer.load_checkpoint(fg_checkpoint_path)
        # valid_loader = fg_NeRFDataset(fg_opt, device=device, type='val', H=fg_opt.H, W=fg_opt.W, size=5).dataloader()
        # fg_trainer.evaluate(valid_loader,'test!')

        print("finish load bg_model and fg_model")
        # return
        with torch.no_grad():
            for i,data in enumerate(loader):
                ...
                # print(f"latents_ubyte.shape{latents_ubyte.shape}")
                ...
                # import pdb; pdb.set_trace()
                # fg_sigma_sum,fg_color_sum=self.composite_function(sigma,color)
                # bg_sigma_sum,bg_color_sum=self.composite_function(sigma1,color1)
                ...
                # get alpha
                # fg_weights=self.calc_volume_weights(di,rays_d,fg_sigma_sum)
                # bg_weights=self.calc_volume_weights(di,rays_d,bg_sigma_sum)
                weights=self.calc_volume_weights(di,rays_d,sigma_sum)

                # (Pdb) p weights.shape
                # torch.Size([1, 25600, 512]) 

                # fg_alpha_map=torch.sum(fg_weights,dim=-1)
                # fg_alpha_map=fg_alpha_map.reshape(res,res).detach().cpu().numpy()
                # bg_alpha_map=torch.sum(bg_weights,dim=-1)
                # bg_alpha_map=bg_alpha_map.reshape(res,res).detach().cpu().numpy()
                # fg_color_map=torch.sum(fg_weights.unsqueeze(-1)*fg_color_sum,dim=-2)
                # bg_color_map=torch.sum(bg_weights.unsqueeze(-1)*bg_color_sum,dim=-2)
                color_map=torch.sum(weights.unsqueeze(-1)*color_sum,dim=-2)
                # import pdb;pdb.set_trace()
                # color_map1=color_map1.reshape(1,res,res,3).permute(0,3,1,2).contiguous()
                # torch.Size([1, 25600, 4])
                # feature_map1=fg_trainer.diffusion.encode_imgs(color_map1)
            
                print(f"color_sum shape {color_sum.shape}")
                # print(f"feature_map1 shape {feature_map1.shape}")
                
                # feature_map=fg_color_map.reshape(-1,res,res,4).permute(0, 3, 1, 2).contiguous()
                # torch.Size([1, 4, 160, 160])
                # 1 1280 1280 3

                # feature=feature_map+feature_map1
                # color_map2=fg_trainer.diffusion.decode_latents(feature_map).permute(0, 2, 3, 1).contiguous()

                # color_map2=fg_trainer.diffusion.decode_latents(feature_map1).permute(0, 2, 3, 1).contiguous()
                ...
                #     transforms.ToTensor(),
                # ])
                # color_map2 = color_map2.squeeze(0)
                # color_map = trans(color_map2.permute(2,0,1)).permute(1,2,0)
                # # pred_rgb2=y.reshape(-1,res,res,3)
                # # print(f"color_map shape {color_map.shape}")
                # # print(f"color_map2 shape {color_map2.shape}")
                # pred_rgb1=color_map.reshape(1,res,res,3)
        ...
        # for i in range(200):
        #     all_preds2[i]=acc_maps[i]*all_preds2[i]+(1-acc_maps[i])*all_preds1[i]
        ...
        print("finish_video")
        # torch.save(pre2, '/data/hychen/ln_data/all_preds0429.pt')
        # for i in range(200):
        #     if i%10==0:
        #         Image.fromarray(all_preds1[i].save(save_path / f"{i}_rgb.png"))
